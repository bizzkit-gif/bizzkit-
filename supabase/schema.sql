...
create policy "Owners delete own business"        on public.businesses for delete using (auth.uid() = owner_id);
...
create policy "Owners create connections" on public.connections for insert with check (
  auth.uid() = (select owner_id from public.businesses where id = from_biz_id)
);
create policy "Owners delete connections" on public.connections for delete using (
  auth.uid() = (select owner_id from public.businesses where id = from_biz_id)
);
...
create policy "Users view own saved"   on public.saved_businesses for select using (auth.uid() = user_id);
create policy "Users save businesses"  on public.saved_businesses for insert with check (auth.uid() = user_id);
create policy "Users unsave"           on public.saved_businesses for delete using (auth.uid() = user_id);
...
create policy "Conferences viewable by everyone" on public.conferences for select using (true);
create policy "Owners create conferences" on public.conferences for insert with check (
  auth.uid() = (select owner_id from public.businesses where id = organizer_id)
);
create policy "Organizers update conferences" on public.conferences for update using (
  auth.uid() = (select owner_id from public.businesses where id = organizer_id)
);
create policy "Organizers delete conferences" on public.conferences for delete using (
  auth.uid() = (select owner_id from public.businesses where id = organizer_id)
);
...
create policy "Attendees viewable by everyone" on public.conference_attendees for select using (true);
create policy "Owners join conferences" on public.conference_attendees for insert with check (
  auth.uid() = (select owner_id from public.businesses where id = business_id)
);
create policy "Owners leave conferences" on public.conference_attendees for delete using (
  auth.uid() = (select owner_id from public.businesses where id = business_id)
);
...
create policy "Participants view chats" on public.chats for select using (
  auth.uid() = (select owner_id from public.businesses where id = participant_a) or
  auth.uid() = (select owner_id from public.businesses where id = participant_b)
);
create policy "Owners create chats" on public.chats for insert with check (
  auth.uid() = (select owner_id from public.businesses where id = participant_a) or
  auth.uid() = (select owner_id from public.businesses where id = participant_b)
);
...
create policy "Participants view messages" on public.messages for select using (
  auth.uid() = (select owner_id from public.businesses where id = sender_id) or
  auth.uid() in (
...
create policy "Owners send messages" on public.messages for insert with check (
...
create policy "Participants view call rooms" on public.call_rooms for select using (
  auth.uid() in (
    select b.owner_id from public.businesses b
    join public.chats c on (c.participant_a = b.id or c.participant_b = b.id)
    where c.id = chat_id
...
create policy "Owners create call rooms" on public.call_rooms for insert with check (
...
declare
  chat_id uuid; a uuid; b uuid;
begin
  if biz_a < biz_b then a := biz_a; b := biz_b; else a := biz_b; b := biz_a; end if;
  select id into chat_id from public.chats where participant_a = a and participant_b = b;
  if chat_id is null then
    insert into public.chats (participant_a, participant_b) values (a, b) returning id into chat_id;
  end if;
  return chat_id;
end;
$$;
